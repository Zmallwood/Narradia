...
namespace Narradia
{
    ...
    void SubDrawerGround::DrawGround(bool doDrawTerritoryBorders) {
        ...
        if (ground_layer_type) {
            ...
            RendererTiles::Get()->UpdateDrawTile(
                ground_layer_type, p->rendids_tile_layers_[tile_coord.x][tile_coord.y], v0, v1, v2,
        ...
        if (hovered_tile.x == tile_coord.x && hovered_tile.y == tile_coord.y) {
            ...
            v3.position.y += 0.03f;
            RendererTiles::Get()->UpdateDrawTile(
                Hash("TileHovered"), p->rendids_tile_layers_[tile_coord.x][tile_coord.y], v0, v1,
        ...
        if (MobTargeting::Get()->GetTargetedMob() == mob && nullptr != mob) {
            ...
            v3.position.y += 0.03f;
            RendererTiles::Get()->UpdateDrawTile(
                Hash("TileTargetedMob"), p->rendids_tile_layers_[tile_coord.x][tile_coord.y], v0,
        ...
        if (doDrawTerritoryBorders) {
            if (this_tile_claimed_by_player != east_tile_claimed_by_player) {
                RendererTiles::Get()->UpdateDrawTile(
                    Hash("PlayerClaimedTileBorderE"),
                    p->rendids_tile_layers_[tile_coord.x][tile_coord.y], v0, v1, v2, v3, normal00,
                    normal10, normal11, normal01);
            }
            if (this_tile_claimed_by_player != south_tile_claimed_by_player) {
                RendererTiles::Get()->UpdateDrawTile(
                    Hash("PlayerClaimedTileBorderS"),
                    p->rendids_tile_layers_[tile_coord.x][tile_coord.y], v0, v1, v2, v3, normal00,
                    normal10, normal11, normal01);
            }
        }
