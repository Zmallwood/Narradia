...
namespace Narradia
{
    ...
    void MouseRotation::UpdateRotationInProgress() {
        if (current_rotation_type_ != RotationTypes::None ||
            Camera::Get()->camera_distance_ == 2.0f) {
            auto canvas_size = GetCanvasSize();
            auto motion_delta = MouseInput::Get()->GetMotionDeltaPickResults();
            auto delta_x =
                -static_cast<float>(motion_delta.x) * rotation_sensitivity_ / canvas_size.width;
            auto delta_y =
                static_cast<float>(motion_delta.y) * rotation_sensitivity_ / canvas_size.height;
