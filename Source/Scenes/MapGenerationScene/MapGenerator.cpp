...
namespace Narradia
{
    void WorldMapGenerator::DoGenerationStep(GenerationSteps generationStep) {
    ...
    void WorldMapGenerator::AddElevation() {
    ...
    void WorldMapGenerator::AddMobs() {
        auto map_area = World::Get()->GetMapAreaAtZLevel(0);
        auto num_boars = 60;
    ...
    void WorldMapGenerator::AddObjects() {
        auto map_area = World::Get()->GetMapAreaAtZLevel(0);
    ...
    void WorldMapGenerator::AddWater() {
        auto map_area = World::Get()->GetMapAreaAtZLevel(0);
        auto area_count = 30;
        for (auto i = 0; i < area_count; i++) {
    ...
    void WorldMapGenerator::AddWaterRivers() {
        auto map_area = World::Get()->GetMapAreaAtZLevel(0);
        ...
        for (auto i = 0; i < river_count; i++) {
            ...
            for (auto k = 0; k < num_sections; k++) {
                            ...
                            if (x_i < MapArea::GetMapSize().width - 1) {
                                if (map_area->GetTile({x_i + 1, y_i})->GetGroundType() !=
                                    ...
                                    map_area->GetTile({x_i + 1, y_i})
                                        ->SetElevation(
                                            map_area->GetTile({x_i, y_i})->GetElevation());
                            }
                ...
                if (next_x >= 0 && next_y >= 0 && next_x < MapArea::GetMapSize().width &&
                    ...
                    current_x < MapArea::GetMapSize().width &&
                    current_y < MapArea::GetMapSize().height) {
    ...
    void WorldMapGenerator::GenerateColorVariations() {
