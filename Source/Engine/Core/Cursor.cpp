...
namespace Narradia
{
    ...
    void Cursor::Render() const {
        ...
        const auto bounds = RectangleF{
            mouse_position.Translate(
                -used_cursor_size.width / 2.0f, -used_cursor_size.height / 2.0f),
            used_cursor_size};
        int image_name_hash;
    ...
    void Cursor::RestoreSavedPosition() const {
        SDL_WarpMouseInWindow(
            Graphics::Get()->GetWindow(), p->saved_mouse_position_pixels_.x,
            p->saved_mouse_position_pixels_.y);
    }
